...
**Done.** `functions/_lib/programValidator.js` was built from `program.schema.json`/`validate_program.py` as the reference implementation, not designed separately. All three now share a `KEY_RE` identifier-charset restriction on `testDefinitions.key`/`warmupTemplates.key`/`sessionTemplates.key` (see Phase D below) — if one changes, change all three.
...
**Phase C — DONE.** Multi-user checklist + the Getting Started tab. `resolveProgram()`/`resolveProgramSourceFlag()` in `app.js` were rewritten to fix a real bug: every path used to fall through to the bundled static ski file regardless of *why* a fetch came back empty, so a second user with no program of their own silently saw Eric's ski program instead of an empty state. Fixed so `content` is `null` only for "genuinely nothing to show" (new user, unauthenticated, unrecoverable error with no cache) and never silently substitutes another program. Branding (browser tab title, top-left `.brand` element) is now driven entirely by the active program's `displayName`, with a generic `"Training Log"` fallback when there's no program yet or `displayName` is absent — never a hardcoded sport name in the markup. The "Start" nav tab (5th button, always visible) hosts the empty state, links to the starter content, the program list, and the upload form — one hub, not a separate settings screen. `functions/api/programs/index.js`'s `GET` and `functions/_lib/db.js`'s `listPrograms`/`getProgramOwner` are scoped by `user_email` the same way every other query is.

**Phase D — DONE.** `functions/_lib/programValidator.js` is the dependency-free JS gatekeeper (ported from `validate_program.py`, kept in sync by hand — see the sync note there): structural checks, every reference-integrity check (dangling video/circuit/test/warmupTemplate/activationGroup, pct-without-lift, rx-outside-scope, non-adjacent `group`), and the sanitization rules — control-character rejection, per-field length caps, a 200KB whole-document size cap, and a `javascript:`/`data:` URL scheme allowlist (`isSafeUrl`, http/https only). 29 dedicated tests in `test/programValidator.test.js`, including deliberate injection attempts, all passing; 82 tests passing project-wide.

New endpoints under `functions/api/programs/`: `GET`/`POST /api/programs` (list, create draft), `POST /api/programs/:id/versions` (append immutable version), `POST /api/programs/:id/activate` (atomic `DB.batch()` archive-old + activate-new). Every mutating endpoint checks `getProgramOwner(db, id) === context.data.userEmail` before touching a program by id — not just that the requester is authenticated, but that they own *this* program — and returns a 404 (not 403) for someone else's id, same as "doesn't exist." All writes are parameterized `.bind()` calls; no string-concatenated SQL anywhere in `db.js`. All three JSON-body endpoints wrap `request.json()` in try/catch (400 on malformed JSON) and return structured `{error, details}` JSON on validation failure (422) or an unexpected error (500), never a raw crash.

**A real XSS gap was found and fixed during this phase's browser testing**, not just unit tests: `app.js` interpolated several program-content strings into `href`/`id`/`data-*` attributes without `esc()` — the warm-up erg-table's time/effort-label cells, the video-link `href` on both the single-exercise and superset rendering paths, and (structurally) `session.key`/`testDefinitions.key`/`warmupTemplates.key`, which flow into DOM element ids (`s.id`, `data-key`, `id="t-${k}..."`) where HTML-escaping an id doesn't help — an escaped id just fails to match the id it names. Fixed two ways: escaped the erg-table cells and video `href`s at render time, and added a `KEY_RE` identifier-charset restriction (`^[a-zA-Z][a-zA-Z0-9_]*$`) for the three key fields in `programValidator.js`, `validate_program.py`, and `program.schema.json` (all three kept in sync), since a key is a programmatic identifier, not display text, and no real program data needs anything outside that charset. Verified end-to-end in a live `wrangler pages dev` session: a payload with a `javascript:` video URL and an `"onmouseover="`-shaped session key was rejected with a clean `422` and two specific errors, no crash, no draft created.

Starter content (`build-your-training-plan.md`, `program.schema.json`, `example-program.json`) is copied into `public/` so it's servable directly from the deployed site and linked from the Getting Started tab; the root-level copies stay the source of truth Claude reads/writes, and both copies must be kept in sync by hand on future edits (no build step does this automatically).

Verified live: a brand-new second user (no programs) gets the generic empty state and auto-lands on Getting Started; uploading `example-program.json` creates a draft, activating it flips `current`/`archived` correctly and the app immediately renders the new program's own sessions/branding; switching back to Eric's real dev identity confirms his `current` ski program and its version history were untouched by the second user's writes throughout.
...
- Phase C: **done.** Verified live in `wrangler pages dev` with `DEV_USER_EMAIL` switched to a second, never-before-seen address: empty/no-program state renders (generic "Training Log" branding, no ski leakage), the app auto-lands on Getting Started instead of a broken Session view, and switching back to Eric's real identity confirmed his `current` program and log were completely unaffected.
- Phase D: **done.** Round-tripped `example-program.json` through upload → validate → draft → activate live; the app immediately rendered the new program's own sessions correctly. Verified sanitization live, not just in unit tests: a payload with a `javascript:` video URL and an `"onmouseover="`-shaped session key was rejected with a clean `422` and two specific errors (no crash, no draft, no XSS execution). Also confirmed the real ski program, `example-program.json`, and the fixture-derived real content all still validate cleanly after the new `KEY_RE` identifier restriction was added.

The manual browser-driven checks above (Phase C/D) are now also captured as a permanent, cheap-to-rerun Playwright suite — see `e2e/README.md`. It exercises the same scenarios (brand-new-user empty state, upload → activate, malicious-payload rejection, cross-user ownership checks) against a real `wrangler pages dev` + local D1, each test as its own throwaway synthetic user via the dev-only `X-Test-User-Email` header (`functions/api/_middleware.js`) — no more re-deriving this verification by hand in a future session. Run via `npm run test:e2e`.
