...
- **Sessions per period:** use as many or as few as the person's schedule needs, with short identifier keys -- letters, numbers, and underscore only, starting with a letter (e.g. `"A"`, `"D1"`, `"Run"`, not `"A/B"` or `"5-day"`).
